...
public class BedrockModelLoader {
    ...
    /**
     * Loads Bedrock animation data from a resource.
     * 
     * @param resource The resource containing the `.animation.json` data
     * @param parser The parser to use
     * @return The parsed {@link BedrockAnimations} object
     */
    public static BedrockAnimations loadAnimations(Resource resource, ConfigParser<Config> parser) {
        Config config = parser.parse(resource.asString());

        BedrockAnimations animations = new BedrockAnimations();
        animations.format_version = config.get("format_version");
        Config animsConfig = config.get("animations");
        if (animsConfig == null) return null;

        animations.animations = new HashMap<>();
        for (Config.Entry entry : animsConfig.entrySet()) {
            if (entry.getValue() instanceof Config animConfig) {
                BedrockAnimations.Animation animation = new BedrockAnimations.Animation();
                animation.animation_length = getFloat(animConfig, "animation_length", 0f);
                Object loop = animConfig.get("loop");
                if (loop instanceof Boolean b) {
                    animation.loop = b;
                } else if (loop instanceof String s) {
                    animation.loop = s.equalsIgnoreCase("true") || s.equalsIgnoreCase("hold_on_last_frame");
                } else {
                    animation.loop = false;
                }
                
                Config bonesConfig = animConfig.get("bones");
                if (bonesConfig != null) {
                    animation.bones = new HashMap<>();
                    for (Config.Entry boneEntry : bonesConfig.entrySet()) {
                        if (boneEntry.getValue() instanceof Config boneAnimConfig) {
                            BedrockAnimations.BoneAnimation boneAnimation = new BedrockAnimations.BoneAnimation();
                            boneAnimation.position = boneAnimConfig.get("position");
                            boneAnimation.rotation = boneAnimConfig.get("rotation");
                            boneAnimation.scale = boneAnimConfig.get("scale");
                            animation.bones.put(boneEntry.getKey(), boneAnimation);
                        }
                    }
                }
                animations.animations.put(entry.getKey(), animation);
            }
        }
