    ...
    public void addTexture(Identifier textureIdentifier, Identifier... atlasIdentifiers) {
        if (atlasIdentifiers.length == 0) {
            ...
            for (Identifier atlasIdentifier : atlasIdentifiers) {
                if (!texturesToCompileToAtlas.containsKey(atlasIdentifier)) Log.crash("Tried to add a texture to a non-existent atlas: " + atlasIdentifier + " make sure that the atlas is registered beforehand");
